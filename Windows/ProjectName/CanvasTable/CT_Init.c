...
int CT_ScreenW = 0; //屏幕宽度
int CT_ScreenH = 0; //屏幕高度
...
void CT_init(int iw,int ih,char **rl,int rl_n,void *CT_main,void *CT_loop,int dm,int sph,int lbh,int ww,int wh,int arg6,int arg7,int arg8,int arg9,int arg10,int arg11,int arg12,int arg13,int arg14,int arg15,int arg16)
{
    ...
    cprint("CanvasTable.ENTITY Version : 0.10.5 Alpha");
    cprint("CanvasTable.ENTITY Build : 2023 / ?? / ??");
    cprint("----------Init----------");
    timeBeginPeriod(1); //设置Sleep()最低分辨率
    ...
    CT_ResourceLoadStart(rl_n); //开始加载资源
...
void CT_WindowCreate()
{
    if(CT_CanvasW > 0 && CT_CanvasH > 0) //如果画布宽度和高度都大于0
    {
        ...
        WC_MainWindow.lpszClassName ="MainWinodw";
        WC_MainWindow.lpszMenuName = NULL;
        WC_MainWindow.style = CS_HREDRAW | CS_VREDRAW;
        RegisterClass(&WC_MainWindow);
        CT_MainWindow = CreateWindow("MainWinodw","",WS_SYSMENU|WS_MINIMIZEBOX,0,0,200,200,NULL,NULL,NULL,NULL); //创建窗口
        ...
        CT_ScreenW = GetSystemMetrics(SM_CXSCREEN); //获取屏幕宽度
        CT_ScreenH = GetSystemMetrics(SM_CYSCREEN); //获取屏幕高度
        CT_WindowX = (CT_ScreenW - (CT_WindowW + winborder_w)) / 2; //窗口位置x
        CT_WindowY = (CT_ScreenH - (CT_WindowH + winborder_h)) / 2; //窗口位置y
