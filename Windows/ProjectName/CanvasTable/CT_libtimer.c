...
void timer_set(char *timer,float time)
{
    ...
    if(time >= 0.1) //如果定时器时间大于等于0.1
    {
        ...
        if(CT_ContainerTimerLength % 1000 == 0) //如果定时器容器长度是1000的倍数
        {
            CT_ContainerPreallocated_Timer(CT_ContainerTimerLength+1000); //预先分配定时器容器
        }
    }
    else
    {
...
void globaltimer_set(char *timer,float time)
{
    ...
    if(time >= 0.1) //如果全局定时器时间大于等于0.1
    {
        ...
        if(CT_ContainerGlobalTimerLength % 1000 == 0) //如果全局定时器容器长度是1000的倍数
        {
            CT_ContainerPreallocated_GlobalTimer(CT_ContainerGlobalTimerLength+1000); //预先分配全局定时器容器
        }
    }
    else
    {
