...
extern int CT_ContainerTileIndex;
...
void scene_enter(CTscene *scene)
{
    ...
    if(scene->type == -1) //如果对象是场景对象
    {
        ...
        if(CT_sceneWidth > 0 && CT_sceneHeight > 0 && CT_GridWidth > 0 && CT_GridHeight) //如果场景大小和网格大小大于0
        {
                                        ...
                                        for(i3=0;i3<ti_index;i3++) //遍历瓦片图块
                                        {
                                            ...
                                            if(ti_gy >= ti_gyn) //如果下一个瓦片图块位置y大于瓦片图像高度
                                            {
                                                ...
                                                if(ti_i > 255) //如果瓦片图块索引没有对应的瓦片图块
                                                {
                                                    ...
                                                    char arg1[8] = "";
                                                    char arg2[8] = "";
                                                    strcat(infor,"ERROR 200 : Beyond range of tile image group. TileImageIndex : ");
                                                    sprintf(arg0,"%d",ti_i);
                                                    strcat(infor,arg0);
                                                    cprint(infor);
                                                    return;
            ...
            {
                char infor[1024] = "";
                strcat(infor,"ERROR 201 : The size of the scene must be greater than the grid size");
                cprint(infor);
            }
        }
        else
        {
            char infor[1024] = "";
            strcat(infor,"ERROR 202 : Unable to enter the scene");
            cprint(infor);
        }
    }
    else
    {
        char infor[1024] = "";
        strcat(infor,"ERROR 203 : The specified scene is not a scene object");
        cprint(infor);
    }
...
void tile_create(int l,int x,int y,int res,int gx,int gy)
{
    ...
    {
        int tix = gx * CT_GridWidth; //瓦片图像位置x
        int tiy = gy * CT_GridHeight; //瓦片图像位置y
        int tile_i = CT_ContainerTileIndex; //tile容器索引
        if(res != -1) //如果有tile图像
        {
