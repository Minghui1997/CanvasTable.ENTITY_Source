        ...
        if(win->type == 0) //如果对象类型是窗口
        {
            ...
            if(w > 0 && h > 0) //如果窗口对象大小正确
            {
                ...
                for(i=0;i<CT_ContainerWindowLength;i++) //遍历窗口容器
                {
                    ...
                    {
                        int win_id = CT_ContainerWindow[i][1]; //获取当前窗口id
                ...
                //将窗口数据写入窗口容器
                CT_ContainerWindow[CT_ContainerWindowLength][0] = 1;
                CT_ContainerWindow[CT_ContainerWindowLength][1] = id;
                CT_ContainerWindow[CT_ContainerWindowLength][2] = l;
                CT_ContainerWindow[CT_ContainerWindowLength][3] = x;
                CT_ContainerWindow[CT_ContainerWindowLength][4] = y;
                CT_ContainerWindow[CT_ContainerWindowLength][5] = w;
                CT_ContainerWindow[CT_ContainerWindowLength][6] = h;
                CT_ContainerWindow[CT_ContainerWindowLength][7] = cbg;
                CT_ContainerWindow[CT_ContainerWindowLength][8] = cbu;
                CT_ContainerWindow[CT_ContainerWindowLength][9] = cbd;
                CT_ContainerWindow[CT_ContainerWindowLength][10] = cbl;
                CT_ContainerWindow[CT_ContainerWindowLength][11] = cbr;
                CT_ContainerWindow[CT_ContainerWindowLength][12] = bi;
                CT_ContainerWindow[CT_ContainerWindowLength][13] = bio;
                win->index = CT_ContainerWindowLength; //窗口对象索引
                CT_ContainerWindowLength ++; //窗口容器索引 +1
                if(CT_ContainerWindowLength % 100 == 0) //如果窗口容器长度为100的倍数
                {
                    CT_ContainerPreallocated_Window(CT_ContainerWindowLength+100); //预先分配窗口容器
                }
                CT_Repaint = 1; //请求重绘
            }
            else
                ...
                if(win->id == Cwin_id) //如果窗口对象id等于容器中指定窗口对象的ID
                {
                    ...
                    for(i2=0;i2<CT_ContainerUILength;i2++) //遍历UI对象容器
                    {
                        if(CT_ContainerUI[i2][0] != NULL) //如果当前UI对象不为空
                        {
                            ...
                            {
                                int ui_type = CT_ContainerUI[i2][3]; //获取当前ui类型
                                int i3;
    ...
    if(CT_RunnerStatus == 1) //如果运行器状态为1
    {
        if(win->type == 0) //如果对象类型是窗口
        {
            ...
            for(i=0;i<CT_ContainerUILength;i++) //遍历UI容器
            {
                ...
                {
                    int ui_id = CT_ContainerUI[i][1]; //获取当前UI id
            ...
            if(type == 1) //如果UI是按钮
            {
                ...
                if(w > 0 && h > 0) //如果按钮大小正确
                {
                    ...
                    //将UI数据写入UI容器
                    CT_ContainerUI[CT_ContainerUILength][0] = 1;
                    CT_ContainerUI[CT_ContainerUILength][1] = id;
                    CT_ContainerUI[CT_ContainerUILength][2] = win_index;
                    CT_ContainerUI[CT_ContainerUILength][3] = type;
                    CT_ContainerUI[CT_ContainerUILength][4] = x;
                    CT_ContainerUI[CT_ContainerUILength][5] = y;
                    CT_ContainerUI[CT_ContainerUILength][6] = w;
                    CT_ContainerUI[CT_ContainerUILength][7] = h;
                    CT_ContainerUI[CT_ContainerUILength][8] = cbg;
                    CT_ContainerUI[CT_ContainerUILength][9] = cbu;
                    CT_ContainerUI[CT_ContainerUILength][10] = cbd;
                    CT_ContainerUI[CT_ContainerUILength][11] = cbl;
                    CT_ContainerUI[CT_ContainerUILength][12] = cbr;
                    CT_ContainerUI[CT_ContainerUILength][13] = ct;
                    CT_ContainerUI[CT_ContainerUILength][14] = bi;
                    strcpy(CT_ContainerUI_Value[CT_ContainerUILength],text);
                    CT_ContainerUI[CT_ContainerUILength][15] = text_w;
                    CT_ContainerUI[CT_ContainerUILength][16] = text_h;
                    CT_ContainerUI[CT_ContainerUILength][17] = 0;
                    CT_ContainerUI[CT_ContainerUILength][18] = 0;
                    CT_ContainerUI[CT_ContainerUILength][19] = 0;
                    CT_ContainerUI[CT_ContainerUILength][20] = 0;
                    CT_ContainerUI[CT_ContainerUILength][21] = font_i;
                    CT_ContainerUI[CT_ContainerUILength][22] = font_n;
                    CT_ContainerUI[CT_ContainerUILength][23] = font_g;
                    CT_ContainerUI[CT_ContainerUILength][24] = bio;
                    ...
                    if(CT_ContainerUILength % 500 == 0) //如果UI容器长度为500的倍数
                    {
                        CT_ContainerPreallocated_UI(CT_ContainerUILength+1000); //预先分配UI容器
                    }
                    CT_Repaint = 1; //请求重绘
                }
                else
            ...
            if(type == 2) //如果UI是文本
            {
                ...
                CT_ContainerUI[CT_ContainerUILength][4] = x;
                CT_ContainerUI[CT_ContainerUILength][5] = y;
                CT_ContainerUI[CT_ContainerUILength][6] = ct;
                strcpy(CT_ContainerUI_Value[CT_ContainerUILength],text);
                CT_ContainerUI[CT_ContainerUILength][7] = font_i;
                CT_ContainerUI[CT_ContainerUILength][8] = font_n;
                CT_ContainerUI[CT_ContainerUILength][9] = font_g;
                CT_ContainerUI_FontCharList_P[CT_ContainerUILength] = font_cl;
                CT_ContainerUI_FontGlythList_P[CT_ContainerUILength] = font_gl;
                ctui->index = CT_ContainerUILength; //UI对象索引
                CT_ContainerUILength ++; //UI容器索引 +1
            ...
            if(type == 3) //如果UI是复选框
            {
                ...
                if(status == 0 || status == 1) //如果复选框属性正确
                {
                    ...
                    CT_ContainerUI[CT_ContainerUILength][7] = h;
                    CT_ContainerUI[CT_ContainerUILength][8] = cbg;
                    CT_ContainerUI[CT_ContainerUILength][9] = cb;
                    CT_ContainerUI[CT_ContainerUILength][10] = cw;
                    CT_ContainerUI[CT_ContainerUILength][11] = status;
                    ctui->index = CT_ContainerUILength; //UI对象索引
                    CT_ContainerUILength ++; //UI容器索引 +1
                    if(CT_ContainerUILength % 500 == 0) //如果UI容器长度为500的倍数
                    {
            ...
            if(type == 4) //如果UI是单选框
            {
                ...
                if(g >= 0) //如果单选框组有效
                {
                    ...
                    CT_ContainerUI[CT_ContainerUILength][9] = cb;
                    CT_ContainerUI[CT_ContainerUILength][10] = cw;
                    CT_ContainerUI[CT_ContainerUILength][11] = g;
                    CT_ContainerUI[CT_ContainerUILength][12] = 0;
                    ctui->index = CT_ContainerUILength; //UI对象索引
                    CT_ContainerUILength ++; //UI容器索引 +1
                    if(CT_ContainerUILength % 500 == 0) //如果UI容器长度为500的倍数
                    {
            ...
            if(type == 5) //如果UI是进度条
            {
                ...
                if(w > 0 && h > 0 && val >= 0 && maxval > 0) //如果进度条大小正确
                {
                    ...
                    CT_ContainerUI[CT_ContainerUILength][11] = cbl;
                    CT_ContainerUI[CT_ContainerUILength][12] = cbr;
                    CT_ContainerUI[CT_ContainerUILength][13] = cw;
                    CT_ContainerUI[CT_ContainerUILength][14] = bi;
                    CT_ContainerUI[CT_ContainerUILength][15] = w_w;
                    CT_ContainerUI[CT_ContainerUILength][16] = val;
                    CT_ContainerUI[CT_ContainerUILength][17] = maxval;
                    CT_ContainerUI[CT_ContainerUILength][18] = bio;
                    ctui->index = CT_ContainerUILength; //UI对象索引
                    CT_ContainerUILength ++; //UI容器索引 +1
                    if(CT_ContainerUILength % 500 == 0) //如果UI容器长度为500的倍数
                    {
            ...
            if(type == 6) //如果UI是文本框
            {
                ...
                if(w > 0 && h > 0 && tpass >= 0 && tnumb >= 0 && tmax > 0) //如果文本框大小正确
                {
                    ...
                    CT_ContainerUI[CT_ContainerUILength][13] = ct;
                    CT_ContainerUI[CT_ContainerUILength][14] = bi;
                    CT_ContainerUI[CT_ContainerUILength][15] = tpass;
                    CT_ContainerUI[CT_ContainerUILength][16] = tnumb;
                    CT_ContainerUI[CT_ContainerUILength][17] = tmax;
                    CT_ContainerUI[CT_ContainerUILength][18] = readonly;
                    CT_ContainerUI[CT_ContainerUILength][19] = subbox;
                    CT_ContainerUI[CT_ContainerUILength][20] = font_i;
                    CT_ContainerUI[CT_ContainerUILength][21] = font_n;
                    CT_ContainerUI[CT_ContainerUILength][22] = font_g;
                    CT_ContainerUI[CT_ContainerUILength][23] = text_h;
                    CT_ContainerUI[CT_ContainerUILength][24] = bio;
                    CT_ContainerUI_FontCharList_P[i2] = font_cl;
                    CT_ContainerUI_FontGlythList_P[i2] = font_gl;
                    strcpy(CT_ContainerUI_Value[CT_ContainerUILength],"");
                    strcpy(CT_ContainerUI_Name[CT_ContainerUILength],name);
                    ctui->index = CT_ContainerUILength; //UI对象索引
                    CT_ContainerUILength ++; //UI容器索引 +1
                    if(CT_ContainerUILength % 500 == 0) //如果UI容器长度为500的倍数
                    {
            ...
            if(type == 7) //如果UI是图像元件
            {
                ...
                int subimg_o = ctui->subimg_o; //图像元件子图像偏移
                int subnum = ctui->subnum; //图像元件子图像数量
                int image_w = 0; //图像元件图像宽度
                int image_h = 0; //图像元件图像高度
                ...
                if(image != -1) //如果图像元件有图像
                {
                    ...
                    if(res_type == 0) //如果资源类型是图像
                    {
                        image_w = CT_ResourceArgList[image][2]; //图像元件图像宽度
                        image_h = CT_ResourceArgList[image][3]; //图像元件图像高度
                        subimg_w = image_w / subnum - subimg_o * 2; //子图像宽度
                    ...
                    if(subnum >= 1 && subimg <= subnum - 1) //如果图像元件属性正确
                    {
                        ...
                        for(i2=0;i2<CT_ContainerUILength;i2++) //遍历UI容器
                        {
                            if(CT_ContainerUI[i2][0] == 0) //如果当前UI为空
                            {
                                //将UI数据写入UI容器
                                CT_ContainerUI[i2][0] = 1;
                                CT_ContainerUI[i2][1] = id;
                                ...
                                CT_ContainerUI[i2][4] = x;
                                CT_ContainerUI[i2][5] = y;
                                CT_ContainerUI[i2][6] = image;
                                CT_ContainerUI[i2][7] = subimg;
                                CT_ContainerUI[i2][8] = subnum;
                                CT_ContainerUI[i2][9] = subimg_w;
                                CT_ContainerUI[i2][10] = subimg_h;
                                CT_ContainerUI[i2][11] = subimg_o;
                                ctui->index = i2; //UI对象索引
                                CT_Repaint = 1; //请求重绘
                                return; //中断
                            }
                        ...
                        CT_ContainerUI[CT_ContainerUILength][4] = x;
                        CT_ContainerUI[CT_ContainerUILength][5] = y;
                        CT_ContainerUI[CT_ContainerUILength][6] = image;
                        CT_ContainerUI[CT_ContainerUILength][7] = subimg;
                        CT_ContainerUI[CT_ContainerUILength][8] = subnum;
                        CT_ContainerUI[CT_ContainerUILength][9] = subimg_w;
                        CT_ContainerUI[CT_ContainerUILength][10] = subimg_h;
                        CT_ContainerUI[CT_ContainerUILength][11] = subimg_o;
                        ctui->index = CT_ContainerUILength; //UI对象索引
                        CT_ContainerUILength ++; //UI容器索引 +1
                        if(CT_ContainerUILength % 500 == 0) //如果UI容器长度为500的倍数
                        {
                    ...
                    {
                        cprint("ERROR : The imagement property have error.");
                    }
                }
            }
        }
...
void imagement_set_image(CTui *ctui,int img,int subnum)
{
    if(CT_RunnerStatus == 1) //如果运行器状态为1
    {
        if(ctui->type == 7) //如果UI对象是图像元件
        {
            if(CT_ContainerUI[ctui->index][0] != NULL) //如果UI容器中指定文本框对象的数据不为空
            {
                ...
                if(ctui->id == Cui_id) //如果UI对象id等于容器中指定UI对象的ID
                {
                    ...
                    if(res_type == 0) //如果资源类型是图像
                    {
                        int image_w = CT_ResourceArgList[img][2]; //图像元件图像宽度
                        int image_h = CT_ResourceArgList[img][3]; //图像元件图像高度
                        int subimg_w = 0; //图像元件子图像宽度
                        int subimg_h = 0; //图像元件子图像高度
                        int subimg_o = CT_ContainerUI[ctui->index][11]; //子图像偏移
                        ...
                        //将图像元件数据写入UI容器
                        CT_ContainerUI[ctui->index][6] = img;
                        CT_ContainerUI[ctui->index][7] = 0;
                        CT_ContainerUI[ctui->index][8] = subnum;
                        CT_ContainerUI[ctui->index][9] = subimg_w;
                        CT_ContainerUI[ctui->index][10] = subimg_h;
                        CT_Repaint = 1; //请求重绘
                    }
                    else
                    {
        ...
        {
            cprint("ERROR : Not is imagement object."); //输出错误
        }
    }
}
//--------------------------------------------------------------------
    ...
    if(CT_RunnerStatus == 1) //如果运行器状态为1
    {
        if(ctui->type == 7) //如果UI对象是图像元件
        {
            if(CT_ContainerUI[ctui->index][0] != NULL) //如果UI容器中指定文本框对象的数据不为空
            {
                ...
                if(ctui->id == Cui_id) //如果UI对象id等于容器中指定UI对象的ID
                {
                    int subnum = CT_ContainerUI[ctui->index][8]; //图像元件子图像数量
                    if(subnum >= 1 && i <= subnum - 1) //如果图像元件参数正确
                    {
                        CT_ContainerUI[ctui->index][7] = i; //设置图像元件子图像
                        CT_Repaint = 1; //请求重绘
                    }
                    else
                    {
                        cprint("ERROR : The imagement argument have error.");
                    }
                }
            }
        }
