...
char CT_KeyChar[1] = {0x00}; //按键字符
int CT_ContainerTouchKey[16][7] = {0}; //触摸键容器
//--------------------------------------------------------------------
//--------------------------------------------------------------------
// 输入监听
//--------------------------------------------------------------------
void CT_InputListener()
{
    ...
    while(GetMessage(&msg,NULL,0,0)) //处理消息列队
    {
        ...
        if(msg.message == WM_MOUSEMOVE) //如果鼠标移动
        {
            ...
            CT_MousePosX = CT_MousePosPoint.x * CT_CanvasCRX; //鼠标位置x
            CT_MousePosY = CT_MousePosPoint.y * CT_CanvasCRY; //鼠标位置y
        }
...
void CT_TouchKeyRender()
{
    ...
    {
        if(CT_ContainerTouchKey[i][0] != NULL) //如果当前触摸键不为空
        {
            ...
            if(ti != -1) //如果触摸键有图像
            {
                CTCanvas_Image(tx,ty,ti); //绘制触摸键图像
            }
        }
    }
}
