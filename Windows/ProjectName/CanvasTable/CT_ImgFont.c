...
void CTCanvas_ImgString(int x,int y,int c,char *str,int font_img,int font_number,int font_gap,char *font_charlist,int *font_glyphlist)
{
    ...
    while(str_i < str_l) //遍历字符串
    {
        ...
        while(tree_end < 2)
        {
            ...
            {
                int gx = font_glyphlist[tree_li*4]; //字形位置x
                int gy = font_glyphlist[tree_li*4+1]; //字形位置y
                int gw = font_glyphlist[tree_li*4+2]; //字形位置w
                int gh = font_glyphlist[tree_li*4+3]; //字形位置h
                CTCanvas_Glyph(cx,y,font_img,gx,gy,gw,gh,c); //绘制字体字形
            ...
            {
                int gx = font_glyphlist[tree_ri*4]; //字形位置x
                int gy = font_glyphlist[tree_ri*4+1]; //字形位置y
                int gw = font_glyphlist[tree_ri*4+2]; //字形位置w
                int gh = font_glyphlist[tree_ri*4+3]; //字形位置h
                CTCanvas_Glyph(cx,y,font_img,gx,gy,gw,gh,c); //绘制字体字形
                cx += gw + font_gap; //绘制下一个字符的位置
                find_char = 1; //已找到字符
                break; //中断
