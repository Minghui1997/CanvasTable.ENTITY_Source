...
void CTsprite_new(CTsprite *ctspr)
{
    ...
    ctspr->color = 0xffffffff; //sprite颜色
            ...
            if(subnum >= 1 && subimg <= subnum - 1) //如果sprite子图像数量大于等于1
            {
                ...
                for(i=0;i<CT_ContainerSpriteLength;i++) //遍历sprite容器
                {
                    if(CT_ContainerSprite[i][0] == NULL) //如果当前sprite为空
                    {
                        ...
                        CT_ContainerSprite[i][22] = color;
                        CT_TaskCreated(i,ctspr->main,ctspr->loop); //创建sprite子程序
                        CT_Repaint = 1; //请求重绘
                        return i; //中断
                    }
                }
                ...
                CT_ContainerSprite[conspr_i][22] = color;
                CT_TaskCreated(conspr_i,ctspr->main,ctspr->loop); //创建sprite子程序
                CT_ContainerSpriteLength ++; //sprite容器索引 +1
                if(CT_ContainerSpriteLength % 1000 == 0) //如果sprite容器长度是1000的倍数
                {
                    CT_ContainerPreallocated_Sprite(CT_ContainerSpriteLength+1000); //预先分配sprite容器
                }
                CT_Repaint = 1; //请求重绘
                return conspr_i; //中断
            }
...
void sprite_set_color(int col)
{
    if(CT_RunnerStatus == 1) //如果运行器状态为1
    {
        if(CT_ContainerSprite[CT_TaskRunIndex][0] != NULL) //如果sprite不为空
        {
            CT_ContainerSprite[CT_TaskRunIndex][22] = col; //设置sprite颜色
            CT_Repaint = 1; //请求重绘
        }
    }
}
...
void isprite_set_color(int si,int col)
{
    if(CT_RunnerStatus == 1) //如果运行器状态为1
    {
        if(si < CT_ContainerSpriteLength) //如果sprite索引小于sprite容器长度
        {
            if(CT_ContainerSprite[si][0] != NULL) //如果指定索引的sprite不为空
            {
                CT_ContainerSprite[si][22] = col; //设置指定索引sprite颜色
                CT_Repaint = 1; //请求重绘
            }
        }
    }
        ...
        if(ctspr->type == -2) //如果对象是sprite
        {
            ...
            for(i=0;i<CT_ContainerSpriteLength;i++) //遍历sprite容器
            {
                if(CT_ContainerSprite[i][0] != NULL) //如果当前sprite不为空
                {
                    ...
                    if(spr_id == id) //如果当前sprite id等于sprite id
                    {
                        CT_ContainerSprite[i][22] = col; //设置当前sprite颜色
                    }
                }
            }
        }
