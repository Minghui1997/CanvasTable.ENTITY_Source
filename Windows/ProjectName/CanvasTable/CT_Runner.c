...
{
    double start_time = CT_RunnerQueryCounter(); //开始时间
    double now_time = 0; //现在时间
    double end_time = start_time + time; //结束时间
    int interval = 0; //时间间隔
    int interval_s = (end_time - start_time) / 1000; //时间间隔(秒)
    if(interval_s >= 2) //如果时间间隔(秒)大于等于2
    {
        Sleep(interval_s-2); //等待
        now_time = CT_RunnerQueryCounter(); //获取现在时间
        interval = end_time - now_time; //时间间隔
        if(interval >= 2000) //如果时间间隔大于等于2000
        {
            Sleep(1); //等待1ms
        }
    }
    while(now_time < end_time) //等待时间结束
    {
        now_time = CT_RunnerQueryCounter(); //获取现在时间
    }
}
...
void CT_RunnerWaitDC(double start_time,int time)
{
    double end_time = start_time + time; //结束时间
    double now_time = 0; //现在时间
    int interval = end_time - start_time; //时间间隔
...
void CT_RunnerStart()
{
    ...
    (*CT_Main)(); //执行Main函数
    now_time = CT_RunnerQueryCounter(); //获取现在时间
    CT_RunnerWaitDC(now_time,next_time-now_time); //等待到下一帧
    CT_RunnerLoop(); //运行器循环
}
//--------------------------------------------------------------------
//--------------------------------------------------------------------
