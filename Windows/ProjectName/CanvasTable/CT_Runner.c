...
{
    double next_frame = start_time + 16667 - CT_SleepDeviation; //下一帧时间
    int interval_ms = (next_frame - now_time) / 1000; //时间间隔(毫秒)
    if(interval_ms >= 0) //时间间隔(毫秒)大于等于0
    {
        Sleep(interval_ms); //等待
    }
    now_time = CT_RunnerQueryCounter(); //获取现在时间
    if(now_time >= next_frame) //如果现在时间大于等于下一帧时间
    {
        CT_SleepDeviation = now_time - next_frame; //Sleep误差
    }
    else
    {
        CT_SleepDeviation = 0; //Sleep误差为0
    }
    while(now_time < next_frame) //如果现在时间小于下一帧时间
    {
        now_time = CT_RunnerQueryCounter(); //获取现在时间
    }
}
