...
double CT_Time = 0; //运行器时间
double CT_NextTime = 0; //下一帧时间
int CT_SleepMS = 0; //Sleep时间 毫秒
double CT_SleepStart = 0; //Sleep开始时间
int CT_SleepDeviation = 0; //Sleep误差
int CT_SleepMaxDeviation = 0; //Sleep最大误差
//--------------------------------------------------------------------
// 运行器查询计数器
//--------------------------------------------------------------------
double CT_RunnerQueryCounter()
{
    ...
    return CT_Counter.QuadPart * 1000000 / CT_Frequency; //获取计数器时间
}
//--------------------------------------------------------------------
//--------------------------------------------------------------------
// 运行器初始化
...
void CT_RunnerInit()
{
    ...
    wglMakeCurrent(NULL,NULL); //取消缓冲画布环境
...
void CT_RunnerStart()
{
    CT_Time = CT_RunnerQueryCounter(); //获取运行器时间
    CT_NextTime = CT_Time + 16667; //下一帧时间
    wglMakeCurrent(CT_DCMW,CT_DCBC); //选择缓冲画布环境
    cprint("CanvasTable Running...");
    (*CT_Main)(); //执行Main函数
    CT_Time = CT_RunnerQueryCounter(); //获取运行器时间
    CT_SleepMS = (CT_NextTime - CT_Time) / 1000 / 2.5; //Sleep时间(毫秒)
    CT_SleepStart = CT_Time; //Sleep开始时间
    if(CT_SleepMS > 0) //如果Sleep时间(毫秒)大于0
    {
        Sleep(CT_SleepMS); //等待
    }
    CT_Time = CT_RunnerQueryCounter(); //获取运行器时间
    CT_SleepMaxDeviation = (CT_Time - CT_SleepStart) - CT_SleepMS * 1000; //Sleep最大误差
    //判断Sleep最大误差的范围
    if(CT_SleepMaxDeviation < 0)
    {
        CT_SleepMaxDeviation = 0;
    }
    if(CT_SleepMaxDeviation >=0 && CT_SleepMaxDeviation <= 1000)
    {
        CT_SleepMaxDeviation = 1000;
    }
    if(CT_SleepMaxDeviation > 1000 && CT_SleepMaxDeviation <= 2000)
    {
        CT_SleepMaxDeviation = 2000;
    }
    if(CT_SleepMaxDeviation > 2000 && CT_SleepMaxDeviation <= 3000)
    {
        CT_SleepMaxDeviation = 3000;
    }
    if(CT_SleepMaxDeviation > 3000 && CT_SleepMaxDeviation <= 4000)
    {
        CT_SleepMaxDeviation = 4000;
    }
    if(CT_SleepMaxDeviation > 4000 && CT_SleepMaxDeviation <= 5000)
    {
        CT_SleepMaxDeviation = 5000;
    }
    if(CT_SleepMaxDeviation > 5000)
    {
        CT_SleepMaxDeviation = 5000;
    }
    CT_Time = CT_RunnerQueryCounter(); //获取运行器时间
    while(CT_Time < CT_NextTime - (CT_SleepMaxDeviation + 1000)) //当运行器时间小于下一帧时间和1毫秒+最大误差
    {
        CT_SleepStart = CT_Time; //Sleep开始时间
        Sleep(1); //等待1毫秒
        CT_Time = CT_RunnerQueryCounter(); //获取运行器时间
        CT_SleepDeviation = CT_Time - CT_SleepStart - 1000; //Sleep误差
        //判断Sleep误差的范围
        if(CT_SleepDeviation < 0)
        {
            CT_SleepDeviation = 0;
        }
        if(CT_SleepDeviation >=0 && CT_SleepDeviation <= 1000)
        {
            CT_SleepDeviation = 1000;
        }
        if(CT_SleepDeviation > 1000 && CT_SleepDeviation <= 2000)
        {
            CT_SleepDeviation = 2000;
        }
        if(CT_SleepDeviation > 2000 && CT_SleepDeviation <= 3000)
        {
            CT_SleepDeviation = 3000;
        }
        if(CT_SleepDeviation > 3000 && CT_SleepDeviation <= 4000)
        {
            CT_SleepDeviation = 4000;
        }
        if(CT_SleepDeviation > 4000 && CT_SleepDeviation <= 5000)
        {
            CT_SleepDeviation = 5000;
        }
        if(CT_SleepDeviation > 5000)
        {
            CT_SleepDeviation = 5000;
        }
        CT_SleepMaxDeviation = CT_SleepDeviation; //Sleep最大误差
        CT_Time = CT_RunnerQueryCounter(); //获取运行器时间
    }
    CT_Time = CT_RunnerQueryCounter(); //获取运行器时间
    while(CT_Time < CT_NextTime) //自旋锁 持续到当前帧结束
    {
        CT_Time = CT_RunnerQueryCounter(); //获取运行器时间
    }
    CT_Time = CT_RunnerQueryCounter(); //获取运行器时间
