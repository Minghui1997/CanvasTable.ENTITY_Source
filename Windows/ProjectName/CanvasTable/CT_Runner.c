...
int CT_ContainerTaskIndex = 0; //子程序容器索引
...
void CT_TaskCreated(int task_i,void *task_m,void *task_l)
{
    ...
    if(task_i + 1 > CT_ContainerTaskIndex)
    {
        CT_ContainerTaskIndex = task_i + 1; //改变子程序索引
    }
}
//--------------------------------------------------------------------
//--------------------------------------------------------------------
...
void CT_RunnerReset()
{
    ...
    for(i=0;i<CT_ContainerTaskIndex;i++) //遍历子程序
    {
        if(CT_ContainerTask[i][0] != NULL) //如果当前子程序不为空
        {
            int var_s = i * 128;
    ...
    CT_ContainerDrawIndex = 0; //Draw容器索引
    CT_DrawLayer = 0; //Draw容器层
    CT_ContainerWindowIndex = 0; //窗口容器索引
    CT_ContainerUIIndex = 0; //UI容器索引
    CT_ContainerSpriteIndex = 0; //Sprite容器索引
    CT_ContainerTileIndex = 0; //Tile容器索引
    CT_ContainerTaskIndex = 0; //子程序索引
    CT_TaskRunIndex = 0; //子程序运行索引
