...
int CT_SleepMaxDeviationUS = -3000; //Sleep最大误差(微秒)
int CT_SleepMaxDeviationMS = 3; //Sleep最大误差(毫秒)
...
void CT_RunnerWaitFrame(double start_time,double now_time)
{
    ...
    if(interval_ms > CT_SleepMaxDeviationMS) //时间间隔(毫秒)大于Sleep最大误差(毫秒)
    {
        ...
        deviation = now_time - start_time - (interval_ms - CT_SleepMaxDeviationMS) * 1000; //Sleep误差
        ...
        if(deviation > CT_SleepMaxDeviationUS) //如果Sleep误差大于Sleep最大误差
        {
            ...
            CT_SleepMaxDeviationMS = CT_SleepMaxDeviationUS / 1000; //Sleep最大误差(毫秒)
            if(CT_SleepMaxDeviationMS < 0) //Sleep最大误差(毫秒)小于0
            {
                CT_SleepMaxDeviationMS = 0; //Sleep最大误差(毫秒)为0
            }
        }
    }
    if(now_time < next_frame - (CT_SleepMaxDeviationUS + 1000)) //如果现在时间与下一帧时间的间隔大于Sleep最大误差
    {
        Sleep(1); //等待1ms
    }
    CT_SleepDeviationReset += 1; //重置Sleep最大误差的计数 +1
    if(CT_SleepDeviationReset == 120) //如果重置Sleep最大误差的计数等于120
    {
        CT_SleepMaxDeviationUS = -3000; //重置Sleep最大误差(微秒)
        CT_SleepMaxDeviationMS = 3; //重置Sleep最大误差(毫秒)
