...
// Sprite容器格式
//--------------------------------------------------------------------
// [[占位符,id,层,x,y,图像索引,子图像索引,子图像数量,子图像宽度,子图像高度,上次位置x,上次位置y,碰撞类型,边缘碰撞,子图像偏移],...]
//--------------------------------------------------------------------
int CT_ContainerSprite[10000][20] = {NULL}; //Sprite容器
int CT_ContainerSpriteIndex = 0; //Sprite索引
        ...
        if(CT_ContainerSprite[i][0] != NULL) //如果当前Sprite不为空
        {
            ...
            if(sl == l) //如果当前Sprite层等于渲染层
            {
                ...
                if(image != -1) //如果sprite图像不为空
                {
                    ...
                    CTCanvas_ImagePart(x-_x,y-_y,image,si_x,si_y,subimg_w,subimg_h); //绘制sprite
                }
            }
        }
    }
