...
extern HDC CT_DCMW;
extern float CT_CanvasCRX;
extern float CT_CanvasCRY;
...
void CTCanvas_Rectangle(int x,int y,int w,int h,int c,int f)
{
    ...
    if(f == 0) //如果是空心
    {
        if(CT_CanvasCRX == 1 || CT_CanvasCRY == 1) //如果画布大小与窗口大小一致
        {
...
void CTCanvas_Line(int x1,int y1,int x2,int y2,int w,int c)
{
    ...
    if(x1 == x2) //如果是竖线
    {
        float line_move_x = w * 0.5; //线偏移x
        float line_move_y = w % 2 * 0.5; //线偏移y
        if(CT_CanvasCRX == 1 || CT_CanvasCRY == 1) //如果画布大小与窗口大小一致
        {
            glBegin(GL_LINES); //开始绘制 线
            glVertex2f(x1+line_move_x,y1+line_move_y); //设置顶点
            glVertex2f(x2+line_move_x,y2+line_move_y); //设置顶点
            glEnd(); //结束绘制 线
        }
        else
        {
            ...
            glVertex2f(x2+line_move_x,y2+line_move_y); //设置顶点
            glEnd(); //结束绘制 线
            glDisable(GL_LINE_SMOOTH); //启用线条抗锯齿
        }
    }
    if(y1 == y2) //如果是横线
    {
        float line_move_y = w * 0.5; //线偏移y
        float line_move_x = w % 2 * 0.5; //线偏移x
        if(CT_CanvasCRX == 1 || CT_CanvasCRY == 1) //如果画布大小与窗口大小一致
        {
            glBegin(GL_LINES); //开始绘制 线
            glVertex2f(x1+line_move_x,y1+line_move_y); //设置顶点
...
void CTCanvas_Circle(int x,int y,int r,int c,int f)
{
    ...
    if(f == 0) //如果是空心
    {
        glLineWidth(1.05); //设置线宽度
        glEnable(GL_LINE_SMOOTH); //启用线条抗锯齿
        glBegin(GL_LINE_LOOP); //开始绘制 闭合线
        for(i=0;i<360;i++)
        {
            theta = i * pi / 180;
            lx = x + (r - 0.5) * cos(theta); //顶点位置x
            ly = y + (r - 0.5) * sin(theta); //顶点位置y
            glVertex2f(lx,ly); //设置顶点
        }
        glEnd(); //结束绘制 闭合线
        glDisable(GL_LINE_SMOOTH); //禁用线条抗锯齿
    }
    if(f == 1) //如果是实心
    {
        ...
        {
            theta = i * pi / 180;
            lx = x + (r - 1.5) * cos(theta); //顶点位置x
            ly = y + (r - 1.5) * sin(theta); //顶点位置y
            glVertex2f(lx,ly); //设置顶点
        }
        glEnd(); //结束绘制 闭合线
        glDisable(GL_LINE_SMOOTH); //禁用线条抗锯齿
...
void CTCanvas_Triangle(int x1,int y1,int x2,int y2,int x3,int y3,int c,int f)
{
    ...
    if(f == 1) //如果是实心
    {
        ...
        glEnd(); //结束绘制 多边形
        glDisable(GL_POLYGON_SMOOTH); //禁用多边形抗锯齿
    }
}
//--------------------------------------------------------------------
//--------------------------------------------------------------------
