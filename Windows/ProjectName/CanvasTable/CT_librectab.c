...
char CT_RectabName[128] = ""; //当前记录表名称
...
{
    if(name[0] != 0x00) //如果记录表名称不为空
    {
        if(charindex(name,'/') == -1 && charindex(name,'*') == -1 && charindex(name,':') == -1 && charindex(name,'?') == -1 && charindex(name,'<') == -1 && charindex(name,'>') == -1 && charindex(name,'|') == -1) //如果记录表名称格式正确
        {
            if(CT_RectabName[0] == 0x00) //如果没有打开记录表
            {
                char rectab_path[128] = "RECTAB/"; //记录表路径
                ...
                if((CT_RectabFile=fopen(rectab_path,"r+")) != NULL) //如果记录表文件夹中有记录表文件
                {
                    ...
                    {
                        char *utf8_str = NULL; //UTF-8字符串
                        CT_RectabData = malloc(rectab_size+1); //为当前记录表数据分配内存
                        CT_RectabData[rectab_size] = 0x00;
                        utf8_str = malloc(rectab_size); //为UTF-8字符串分配内存
                        fseek(CT_RectabFile,3,SEEK_SET); //文件指针移至记录表起始
                        fread(utf8_str,rectab_size,1,CT_RectabFile); //读取记录表数据
                        CT_UTF8toMBCS(CT_RectabData,utf8_str); //将UTF-8字符串转换为MBCS字符串
            ...
            {
                char infor[1024] = "";
                strcat(infor,"ERROR 22 : The record table is opened : ");
                strcat(infor,CT_RectabName);
                cprint(infor);
            }
        }
        else
        {
            char infor[1024] = "";
            strcat(infor,"ERROR 10 : The record table name is error : ");
            strcat(infor,name);
            cprint(infor);
        }
    }
...
int rectab_read(char *dest,char *title)
{
    if(title[0] != 0x00) //如果记录表标题不为空
    {
        ...
        {
            char infor[1024] = "";
            strcat(infor,"ERROR 11 : The record table title is error : ");
            strcat(infor,title);
            cprint(infor);
        }
    }
        ...
        if(charindex(title,'&') == -1 && charindex(title,'=') == -1) //如果标题没有 "&" 和 "="
        {
            if(charindex(text,'&') == -1 && charindex(text,'=') == -1) //如果参数文本没有 "&" 和 "="
            {
                ...
                else
                {
                    ...
                    {
                        char replace_data[512] = ""; //替换的数据
                        char replace_part[512] = ""; //替换的部分
                        char new_part[640] = ""; //新部分
                        int data_pos = title_pos + strlen(_title); //记录表中数据的位置
                        int i = data_pos; //数据位置
                        int data_end_pos = 0; //数据结束的位置
                        int replace_part_l = 0; //替换的部分的长度
            ...
            {
                char infor[1024] = "";
                strcat(infor,"ERROR 12 : The record table text is error : ");
                strcat(infor,text);
                cprint(infor);
            }
        }
