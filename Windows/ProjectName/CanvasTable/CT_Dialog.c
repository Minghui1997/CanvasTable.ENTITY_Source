...
void CT_DialogCreate(char *val,char *name,int max,int pass,int numb)
{
    ...
    if(CT_DialogShow == 0) //如果对话框没有显示
    {
        ...
        SendMessage(CT_DialogTextbox,EM_SETLIMITTEXT,max,TRUE); //设置文本框最大字符数
...
{
    if(CT_ContainerUI[CT_Dialog_Index*25] == 1) //如果对话框对应的文本框不为空
    {
        char *uival_p = CT_ContainerUI_Value + CT_Dialog_Index * 1025; //UI容器指针
        Edit_GetText(CT_DialogTextbox,uival_p,CT_BoxMax); //将对话框输入的内容返回给文本框
...
LRESULT CALLBACK CT_DialogProc(HWND CT_DialogWindow,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
    ...
    if(uMsg == WM_COMMAND)
    {
        ...
        {
            if(lParam == CT_DialogButton) //如果点击返回按钮
            {
                CT_DialogReturn(); //对话框返回
            }
        }
    }
    if(uMsg == WM_PAINT) //如果窗口重绘
    {
