    ...
    for(i=0;i<rl_n;i++) //遍历资源列表
    {
        if(CT_ResourceList[i][0] != NULL) //当前文件路径不为空
        {
            ...
            if(charindex(res_name,'.') != -1) //如果当前资源文件名中有"."
            {
                ...
                {
                    char infor[1024] = "";
                    strcat(infor,"ERROR : The resource file name is error : ");
            ...
            {
                char infor[1024] = "";
                strcat(infor,"ERROR : The resource file name is not support : ");
                strcat(infor,CT_ResourceList[i]);
                cprint(infor);
                res_faild = 1;
                break;
...
int CT_ResourceLoad_BMP(char *path)
{
    ...
    if(bmp_file != NULL) //如果BMP文件指针不为空
    {
        ...
        CT_ContainerResource = realloc(CT_ContainerResource,sizeof(char*)*(CT_ResLength+1)); //资源容器重新分配内存
        CT_ContainerResource[CT_ResLength] = malloc(img_w * img_h * 4); //为资源容器中的资源分配内存
        ...
        glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_S,GL_CLAMP); //设置纹理环绕
        glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_T,GL_CLAMP); //设置纹理环绕
        glTexImage2D(GL_TEXTURE_2D,0,4,img_w,img_h,0,GL_RGBA,GL_UNSIGNED_BYTE,CT_ContainerResource[CT_ResLength]); //加载纹理
        free(CT_ContainerResource[CT_ResLength]); //释放资源
        CT_ResourceArgList = realloc(CT_ResourceArgList,sizeof(int*)*(CT_ResLength+1)); //资源列表重新分配内存
        CT_ResourceArgList[CT_ResLength] = malloc(20); //为资源列表中的项分配内存
        CT_ResourceArgList[CT_ResLength][0] = 1; //存入资源占位符
        CT_ResourceArgList[CT_ResLength][1] = 0; //存入资源参数
        CT_ResourceArgList[CT_ResLength][2] = img_w; //存入图像宽度
        CT_ResourceArgList[CT_ResLength][3] = img_h; //存入图像高度
        CT_ResourceArgList[CT_ResLength][4] = tex_id; //存入纹理id
...
int CT_ResourceLoad_WAV(char *path)
{
    ...
    if(wav_file != NULL) //如果当前资源文件存在
    {
        ...
        CT_ContainerResource = realloc(CT_ContainerResource,sizeof(char*)*(CT_ResLength+1)); //资源列表重新分配内存
        CT_ContainerResource[CT_ResLength] = malloc(pcm_size); //为资源容器分配内存
        fseek(wav_file,pcm_data_i,SEEK_SET); //WAV文件定位到PCM数据位置
        fread(CT_ContainerResource[CT_ResLength],pcm_size,1,wav_file); //加载PCM数据
        ...
        CT_ResourceArgList[CT_ResLength] = malloc(20); //为资源列表中的项分配内存
        CT_ResourceArgList[CT_ResLength][0] = 1; //存入资源占位符
        CT_ResourceArgList[CT_ResLength][1] = 1; //存入资源参数
        CT_ResourceArgList[CT_ResLength][2] = CT_DS_Length; //存入DS列表索引
        CT_loadBarWidget(); //加载条循环
        CT_ResLength ++; //资源长度 +1
...
int CT_ResourceLoad_Text(char *path)
{
    ...
    if(text_file != NULL) //如果当前资源文件存在
    {
        ...
        CT_ResourceArgList = realloc(CT_ResourceArgList,sizeof(int*)*(CT_ResLength+1));
        CT_ResourceArgList[CT_ResLength] = malloc(20);
        CT_ResourceArgList[CT_ResLength][0] = 1; //存入资源占位符
        CT_ResourceArgList[CT_ResLength][1] = 2; //存入资源参数
        CT_loadBarWidget(); //加载条循环
        CT_ResLength ++; //资源长度 +1
        strcat(infor,"Resource load : ");
        strcat(infor,path);
...
void CT_LoadBarShow(int n)
{
    ...
    {
        CTCanvas_ImgString(title_x,title_y,bar_c,title,CT_DF_Img,CT_DF_Number,CT_DF_Gap,CT_DF_CharList,CT_DF_GlyphList); //绘制加载条标题
        CTCanvas_Rectangle(bar_x,bar_y,400,30,bar_c,0); //绘制加载条
        CTCanvas_Rectangle(bar_x+1,bar_y+1,398,28,bar_c,0); //绘制加载条
        if(n >= 1 && n <= 14) //如果加载条块索引在指定范围内
        {
            lump_x = bar_x + 4 + (n - 1) * lump_w; //加载条块位置x
            lump_y = bar_y + 4; //加载条块位置y
            CTCanvas_Rectangle(lump_x,lump_y,lump_w,lump_h,bar_c,1); //绘制加载条块
        }
    }
...
void CT_LoadBarShowError(int c,char *s)
{
    ...
    if(CT_LOADBAR_SHOW == 1) //如果加载条显示
    {
        if(c == 0) //如果错误代码是0
        {
            ...
            strcat(err,s);
            CTCanvas_ImgString(err_x,err_y,err_c,err,CT_DF_Img,CT_DF_Number,CT_DF_Gap,CT_DF_CharList,CT_DF_GlyphList); //绘制错误信息
            CTCanvas_Output(); //输出缓冲画布
        }
    }
}
