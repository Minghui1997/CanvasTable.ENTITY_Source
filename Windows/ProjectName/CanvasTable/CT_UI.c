                    ...
                    if(CT_ContainerUI[i2][0] != NULL) //如果当前UI对象不为空
                    {
                        ...
                        if(ui_win_index == win_index) //如果当前UI对象对应窗口索引等于当前窗口索引
                        {
                            ...
                            if(ui_type == 4) //如果当前UI对象是单选框
                            {
                                ...
                                int r_cb2 = (r_cb & 0xffffff00) + (int)(float)((r_cb & 0x000000ff) * 0.8); //单选框边框颜色(抗锯齿)
                                int r_cbg2 = (r_cbg & 0xffffff00) + (int)(float)((r_cbg & 0x000000ff) * 0.8); //单选框背景颜色(抗锯齿)
                                int r_cw2 = (r_cw & 0xffffff00) + (int)(float)((r_cw & 0x000000ff) * 0.8); //单选框部件颜色(抗锯齿)
                                CTCanvas_Circle(r_x+8,r_y+8,r_w/2-1,r_cb,1); //绘制单选框边框
                                CTCanvas_Circle(r_x+8,r_y+8,r_w/2,r_cb2,1); //绘制单选框边框(抗锯齿)
                                CTCanvas_Circle(r_x+8,r_y+8,r_w/2-3,r_cbg,1); //绘制单选框背景
                                CTCanvas_Circle(r_x+8,r_y+8,r_w/2-2,r_cbg2,1); //绘制单选框背景(抗锯齿)
                                if(r_s == 1) //如果单选框状态为1
                                {
                                    CTCanvas_Circle(r_x+8,r_y+8,r_w/2-5,r_cw,1); //绘制单选框部件
                                    CTCanvas_Circle(r_x+8,r_y+8,r_w/2-4,r_cw2,1); //绘制单选框部件(抗锯齿)
                                }
                            }
