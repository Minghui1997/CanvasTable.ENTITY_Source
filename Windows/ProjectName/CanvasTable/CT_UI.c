...
//--------------------------------------------------------------------
int CT_UIId = 0; //UI id
int CT_ContainerWindow[100][15] = {NULL}; //窗口容器
int CT_ContainerWindowIndex = 0; //窗口容器索引
int CT_ContainerUI[1000][25] = {NULL}; //UI容器
char CT_ContainerUI_Value[1000][2048] = {NULL}; //UI值容器
char CT_ContainerUI_Name[1000][64] = {NULL}; //UI名称索引
char *CT_ContainerUI_FontCharList_P[1000] = {NULL}; //UI字体字符列表容器
int *CT_ContainerUI_FontGlythList_P[1000] = {NULL}; //UI字体字形列表容器
int CT_ContainerUIIndex = 0; //UI容器索引
...
void CT_UIRun()
{
    ...
    if(mouse_up(mkey_left)) //如果鼠标松开
    {
        ...
        {
            if(CT_ContainerUI[CT_Button_Index][0] != NULL) //如果按了按钮的按钮不为空
            {
                                        ...
                                        if(ui_type == 6)
                                        {
                                            ...
                                            if(subbox == 1 || CT_DEVICE == 1) //如果使用子文本框或设备是移动平台
                                            {
                                                ...
                                                if(readonly == 0) //如果文本框不是只读
                                                {
                                                    ...
                                                    char *t_name = CT_ContainerUI_Name[i2];
    ...
    if(CT_Textbox_Index != -1) //如果点击了文本框
    {
        ...
        {
            int readonly = CT_ContainerUI[CT_Textbox_Index][19]; //文本框只读参数
            if(readonly == 0) //如果文本框不是只读
            {
                if(CT_KeyChar[0] != 0x00 && CT_KeyChar[0] != 0x08)
                {
                    int tbox_max = CT_ContainerUI[CT_Textbox_Index][17]; //文本框最大字符数
                    int tbox_number = CT_ContainerUI[CT_Textbox_Index][16]; //数字文本框参数
                    int tbox_length = strlen(CT_ContainerUI_Value[CT_Textbox_Index]);
                    if(tbox_length < tbox_max) //如果文本框字符串长度小于文本框最大字符数
                    {
                        if(tbox_number != 1) //如果文本框不是数字文本框
                        {
                            strcat(CT_ContainerUI_Value[CT_Textbox_Index],CT_KeyChar); //文本框字符串增加字符
                            CT_InterfaceRepaint(); //请求重绘
                        }
                        else //如果文本框是数字文本框
                        {
                            if(CT_KeyChar[0] == 48 || CT_KeyChar[0] == 49 || CT_KeyChar[0] == 50 || CT_KeyChar[0] == 51 || CT_KeyChar[0] == 52 || CT_KeyChar[0] == 53 || CT_KeyChar[0] == 54 || CT_KeyChar[0] == 55 || CT_KeyChar[0] == 56 || CT_KeyChar[0] == 57) //如果当前输入的字符是数字
                            {
                                strcat(CT_ContainerUI_Value[CT_Textbox_Index],CT_KeyChar); //文本框字符串增加字符
                                CT_InterfaceRepaint(); //请求重绘
                            }
    ...
    for(i=0;i<CT_ContainerWindowIndex;i++) //遍历窗口容器
    {
        ...
        if(CT_ContainerWindow[i][0] != NULL) //如果当前窗口不为空
        {
            ...
            if(window_layer == l) //如果当前窗口层等于渲染层
            {
                ...
                {
                    CTCanvas_Rect(win_x,win_y,win_w,win_h,win_cbg,1); //绘制窗口背景
                    CTCanvas_Rect(win_x,win_y-2,win_w,2,win_cbu,1); //绘制窗口上边框
                    CTCanvas_Rect(win_x,win_y+win_h,win_w,2,win_cbd,1); //绘制窗口下边框
                    CTCanvas_Rect(win_x-2,win_y-2,2,win_h+4,win_cbl,1); //绘制窗口左边框
                    CTCanvas_Rect(win_x+win_w,win_y-2,2,win_h+4,win_cbr,1); //绘制窗口右边框
                }
                for(i2=0;i2<CT_ContainerUIIndex;i2++) //遍历UI容器
                {
                    if(CT_ContainerUI[i2][0] != NULL) //如果当前UI对象不为空
                    {
                        ...
                        if(ui_win_index == win_index) //如果当前UI对象对应窗口索引等于当前窗口索引
                        {
                            ...
                            if(ui_type == 1) //如果当前UI对象是按钮
                            {
                                ...
                                {
                                    CTCanvas_Rect(b_x,b_y,b_w,b_h,b_cbg,1); //绘制按钮背景
                                    CTCanvas_Rect(b_x,b_y,b_w,2,b_cbu,1); //绘制按钮上边框
                                    CTCanvas_Rect(b_x,b_y+b_h-2,b_w,2,b_cbd,1); //绘制按钮下边框
                                    CTCanvas_Rect(b_x,b_y,2,b_h-2,b_cbl,1); //绘制按钮左边框
                                    CTCanvas_Rect(b_x+b_w-2,b_y,2,b_h-2,b_cbr,1); //绘制按钮右边框
                                }
                                CT_BufferImgString(b_tx,b_ty,b_ct,b_text,b_font_img,b_font_charnum,b_font_chargap,b_font_charlist,b_font_glyphlist); //绘制按钮文本
                            }
                            if(ui_type == 2) //如果当前UI对象是文本
                            {
                                ...
                                CT_BufferImgString(t_x,t_y,t_ct,t_text,t_font_img,t_font_charnum,t_font_chargap,t_font_charlist,t_font_glyphlist); //绘制文本
                            }
                            if(ui_type == 3) //如果当前UI对象是复选框
                            {
                                ...
                                int c_s = CT_ContainerUI[i2][11]; //复选框状态
                                CTCanvas_Rect(c_x,c_y,c_w,c_h,c_cb,1); //绘制复选框外边框
                                CTCanvas_Rect(c_x+2,c_y+2,c_w-4,c_h-4,c_cbg,1); //绘制复选框内边框
                                if(c_s == 1) //如果复选框状态为１
                                {
                            ...
                            if(ui_type == 5) //如果当前UI对象是进度条
                            {
                                ...
                                {
                                    CTCanvas_Rect(p_x,p_y,p_w,p_h,p_cbg,1); //绘制进度条背景
                                    CTCanvas_Rect(p_x,p_y,p_w,2,p_cbu,1); //绘制进度条上边框
                                    CTCanvas_Rect(p_x,p_y+p_h-2,p_w,2,p_cbd,1); //绘制进度条下边框
                                    CTCanvas_Rect(p_x,p_y,2,p_h-2,p_cbl,1); //绘制进度条左边框
                                    CTCanvas_Rect(p_x+p_w-2,p_y,2,p_h-2,p_cbr,1); //绘制进度条右边框
                                }
                                CTCanvas_Rect(p_x+4,p_y+4,p_ww,p_h-8,p_cw,1); //绘制进度条部件
                            }
                            if(ui_type == 6) //如果当前UI对象是文本框
                            {
                                ...
                                {
                                    CTCanvas_Rect(t_x,t_y,t_w,t_h,t_cbg,1); //绘制文本框背景
                                    CTCanvas_Rect(t_x,t_y,t_w,2,t_cbu,1); //绘制文本框上边框
                                    CTCanvas_Rect(t_x,t_y+t_h-2,t_w,2,t_cbd,1); //绘制文本框下边框
                                    CTCanvas_Rect(t_x,t_y,2,t_h-2,t_cbl,1); //绘制文本框左边框
                                    CTCanvas_Rect(t_x+t_w-2,t_y,2,t_h-2,t_cbr,1); //绘制文本框右边框
                                }
                                if(t_pass != 1) //如果文本框不是密码文本框
                                {
                                    CT_BufferImgString(t_x+5,t_ty,t_ct,t_text,t_font_img,t_font_charnum,t_font_chargap,t_font_charlist,t_font_glyphlist); //绘制文本框文本
                                }
                                else //如果文本框是密码文本框
                                {
                                    int t_sl = strlen(t_text); //文本长度
                                    int passi;
                                    for(passi=0;passi<t_sl;passi++) //遍历文本框字符串
                                    {
                                        if(t_text[passi] != 0x00) //如果当前字符不为空
                                        {
                                            strcat(pass_str,"●"); //增加密码符
                                            if(t_text[passi] < 0) //如果当前字符非ascii字符
                                            {
                                                passi ++; //密码文本框字符串索引 +1
                                            }
                                        }
                                    }
                                    CT_BufferImgString(t_x+5,t_ty,t_ct,pass_str,t_font_img,t_font_charnum,t_font_chargap,t_font_charlist,t_font_glyphlist); //绘制文本框文本
                                }
                                if(i2 == CT_Textbox_Index) //如果选择的文本框是当前文本框
                                {
                                    int val_width = 0; //文本框字符串宽度
                                    if(t_pass != 1) //如果文本框不是密码文本框
                                    {
                                        val_width = CT_ImgStringMeasure(t_font_charnum,t_font_chargap,t_font_charlist,t_font_glyphlist,t_text); //文本框字符串宽度
                                    }    
                                    else //如果文本框是密码文本框
                                    {
                                        val_width = CT_ImgStringMeasure(t_font_charnum,t_font_chargap,t_font_charlist,t_font_glyphlist,pass_str); //文本框密码串宽度
                                    }
                                    if(t_readonly == 0) //如果文本框不是只读
                                    {
                                        CTCanvas_Line(t_x+4+val_width+2,t_y+3,t_x+4+val_width+2,t_y+t_h-4,1,color(0,0,0,255)); //绘制文本框输入线
                                    }
                                }
                            }
                        }
