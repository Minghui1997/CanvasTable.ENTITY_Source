        ...
        if(CT_ContainerDraw[i][0] != NULL) //如果当前draw不为空
        {
            ...
            if(draw_l == CT_DrawLayer) //如果当前draw层等于draw层
            {
                ...
                if(draw_type == 4 || draw_type == 9) //如果draw类型是字符串
                {
                    ...
                    CT_ContainerDraw_FontGlyphList_P[i] = NULL;
                }
            }
        }
    }
...
void draw_rect(int x,int y,int w,int h,int c,int f)
{
    ...
    if(CT_ContainerDrawLength % 1000 == 0) //如果draw容器长度为1000的倍数
    {
        CT_ContainerPreallocated_Draw(CT_ContainerDrawLength+1000); //预先分配draw容器
    }
    CT_Repaint = 1; //请求重绘
}
//--------------------------------------------------------------------
